...
from apps.base.forms.quzpide_form import QuzpideDesignForm
from apps.base.models import Answer, Invoice, MeliPost
from apps.base.models.customuser_config import UserConfig
from apps.base.models.quzpide import QuzpideSale, DesignFile
from apps.base.views.auth_view import check_meli_session
from apps.base.views.father_view import FatherCreateView
...
# ========================================================================== #
class QuzpideSaleListView(TemplateView):
    template_name = 'quzpide/list.html'
        ...
        with ApiClient() as api_client:
            ...
            try:
                ...
                for pack_id in meli_sales:
                    ...
                    is_quzpide = False
                    for product in response['order_items']:
                        if MeliPost.objects.filter(meli_id=product['item']['id']).exists():
                            is_quzpide = True
                            break
                    if is_quzpide:
                        s = {
                        ...
                        try:
                            ...
                            if response['status'] == 'delivered':
                                s['date_delivered'] = datetime.fromisoformat(
                                    response['status_history']['date_delivered'])
...
class QuzpideSaleChatView(TemplateView):
    template_name = 'quzpide/modal_chat.html'

    def get_context_data(self, **kwargs):
        context = super(QuzpideSaleChatView, self).get_context_data(**kwargs)
...
class QuzpideDesignCreateView(FatherCreateView):
    model = QuzpideSale
    form_class = QuzpideDesignForm
    template_name = 'quzpide/design.html'
    success_message = 'Success: Design sent.'
    extra_context = {'design_fields': {'meli_username': 'Username', 'meli_sale': 'Venta'}}

    def get_initial(self):
        initial = super(QuzpideDesignCreateView, self).get_initial()
        print('initial')
        check_meli_session()
        ...
        initial['meli_sale'] = self.kwargs['meli_sale']
        return initial

    def get_context_data(self, **kwargs):
        context = super(QuzpideDesignCreateView, self).get_context_data(**kwargs)
        context['meli_sale'] = self.kwargs['meli_sale']
        return context

    def get_success_url(self):
        return reverse_lazy('Quzpide:success', kwargs={'meli_sale': self.kwargs['meli_sale']})


# ========================================================================== #
class QuzpideDesignSuccessView(TemplateView):
    template_name = 'quzpide/success.html'
...
def CreateImageView(request):
    if request.method == 'POST':
        data = request.POST
        files = request.FILES
        album, created = QuzpideSale.objects.get_or_create(meli_sale=data.get('album_id', None))
        image = DesignFile(name=data.get('image_name', None), design_file=files.get('image', None), album=album,
                          position=data.get('image_position', None))
        image.save()
